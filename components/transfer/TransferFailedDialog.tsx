  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-sm mx-auto bg-white">
        ...
        <div className="bg-[#7a0303] rounded-md p-4 mt-4">
          <p className="text-sm font-medium text-white mb-1">Reason:</p>
          <p className="text-sm text-white">{reason}</p>
        </div>
        <Link href="/">
          <button className="w-full mt-6 py-3 rounded-2xl bg-[#223e99] text-white font-semibold text-base transition-opacity hover:opacity-90 active:scale-[0.98]">Close</button>
        </Link>
      </DialogContent>
    </Dialog>
  );
