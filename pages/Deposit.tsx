    ...
    <div className="min-h-screen bg-background pb-28">
      ...
      <section className="px-5 mb-6 animate-fade-up stagger-1">
        <div className="flex items-center gap-2 mb-4">
          {[1, 2, 3].map(s => (
            <div key={s} className={`flex-1 h-1 rounded-full transition-colors ${s <= step ? 'bg-[#034466]' : 'bg-border'}`} />
          ))}
        </div>
      </section>
      ...
      {step === 1 && (
        <>
            ...
            <div className="space-y-3">
              {accounts.map(account => (
                ...
                >
                  ...
                  <p className="font-semibold text-foreground">${account.balance.toLocaleString('en-IE', { minimumFractionDigits: 2 })}</p>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="px-5 animate-fade-up stagger-4">
            <button onClick={() => setStep(2)} disabled={!selectedAccount || !amount} className="w-full py-4 rounded-2xl bg-[#223e99] text-[#034466]-foreground font-semibold disabled:opacity-50">
              Continue
            </button>
          </section>
        </>
      ...
      {step === 2 && (
        <>
          ...
          <section className="px-5 animate-fade-up stagger-2">
            <button onClick={() => setStep(3)} disabled={!frontCaptured || !backCaptured} className="w-full py-4 rounded-2xl bg-[#223e99] text-[#034466]-foreground font-semibold disabled:opacity-50">
              Submit Deposit
            </button>
          </section>
        </>
      ...
      {step === 3 && (
        <section className="px-5 animate-fade-up">
          <div className="banking-card text-center py-12">
            ...
            <div className="bg-[#034466] rounded-xl p-4 mb-6">
            ...
            <Link href="/" className="inline-block w-full py-4 rounded-2xl bg-white border border-gray-200 font-semibold text-foreground hover:bg-[#034466] transition-colors">
              Back to Home
            </Link>
          </div>
        </section>
